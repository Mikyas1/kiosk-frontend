                ...
                <v-layout v-if="show && !isInventoryNull" row wrap>
                    ...
                    <v-container>
                        ...
                        <div primary-title>
                            ...
                            >Edit Item. You have {{ storeToken }} Tokens</div>
                        </div>
                                    ...
                                    >
                                        ...
                                        <v-form ref="feature">
                                            <v-layout v-for="feature in item.features" :key="feature.id" row wrap>
                                                <v-flex xs3 md3 class="pt-4 pl-2">{{ feature.name }}</v-flex>
                                                <v-flex xs9 md9>
                                                <v-text-field
                                                    :label="' ' + feature.name"
                                                    class="input-group--focused pr-4 pl-2 mt-2"
                        ...
                        >
                            <v-icon small color="c_selected_btn">info</v-icon> If you update any item information above, use the button bellow to submit.</p>
                        <v-btn 
                            flat 
                            raised 
                            dark 
                            class="c_selected_btn ml-4 mb-5 text-capitalize mt-2"
                            ...
                            <v-flex xs12 md12>
                                ...
                                >
                                    ...
                                    <v-layout row wrap class="mb-2">
                                        <v-flex v-if="getMainImageHere(item).path" xs12 md2 class="mt-2 mx-2 mb-1">
                                            <EditImage
                                    ...
                                    <v-layout row wrap
                                        v-bind:class="$vuetify.breakpoint.smAndUp && 'mb-1' || 'mb-3'"
                                    >

                                    <v-flex 
                                        v-if="getListImageHere(item).length < 4"
                                        xs12 md2 class="mt-2 ml-2 mb-1 mr-4">
                                        <!-- main image -->
                                        <v-btn
                                        raised
                                        dark
                                    ...
                                    <v-flex v-for="image in getListImageHere(item)" :key="image.id" xs12 md2 class="mt-2 ml-2 mb-1 mr-4">
                                        <EditImage
                                        v-bind:image="image"
                                        v-bind:main="false"
                                        v-bind:itemId="item.id"
...
export default {
    ...
    computed: {
        ...
        childLink() {
            if(this.show) {
               return {
                    name: 'edit-item - ' + this.$route.params.id
                };
            }
            return {
                name: 'edit-item'
    ...
    methods: {
        ...
        uploadNewImages() {
            ...
            this.$store
            .dispatch("dashboard/upload_item_image", {data: formData, id: this.item.id})
            .then((response) => {
                this.$store.commit("SET_SNACKBAR", {
